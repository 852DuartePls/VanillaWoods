package com.duarte.morewood.blocks.pos;
...
public class StrippedPostBlock extends RotatedPillarBlock implements Woodwood, IWaterLoggable {
    ...
    public StrippedPostBlock(final WoodType woodType) {
        super(AbstractBlock.Properties.create(Material.WOOD).hardnessAndResistance(2.0F).sound(SoundType.WOOD));
    ...
    public BlockState getStateForPlacement(final BlockItemUseContext blockItemUseContext) {
        ...
        return this.getDefaultState().with(AXIS, blockItemUseContext.getFace().getAxis())
                .with(DOWN, down instanceof StrippedPostBlock && axis != Direction.Axis.Y)
                .with(UP, up instanceof StrippedPostBlock && axis != Direction.Axis.Y)
                .with(NORTH, north instanceof StrippedPostBlock && axis != Direction.Axis.Z)
                .with(EAST, east instanceof StrippedPostBlock && axis != Direction.Axis.X)
                .with(SOUTH, south instanceof StrippedPostBlock && axis != Direction.Axis.Z)
                .with(WEST, west instanceof StrippedPostBlock && axis != Direction.Axis.X)
    ...
    public BlockState updatePostPlacement(final BlockState blockState0, final Direction direction, final BlockState blockState1, final IWorld world, final BlockPos blockPos, final BlockPos blockPos1) {
        return blockState0.with(SixWayBlock.FACING_TO_PROPERTY_MAP.get(direction),
                blockState1.getBlock() instanceof StrippedPostBlock && direction.getAxis() != blockState0.get(AXIS));
    }
}
